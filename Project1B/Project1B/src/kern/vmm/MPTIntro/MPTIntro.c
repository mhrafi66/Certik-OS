...
char *pageDirEntry=PDirPool[proc_index][pde_index];
return (int)pageDirEntry;
...
PDirPool[proc_index][pde_index] = (char *)((int)IDPTb1[pde_index] | PT_PERM_PTU);
...
int *pageAddress = (int *)((int)PDirPool[proc_index][pde_index] & 0xfffff000 );
//Incomplete
return (int) pageAddress[pte_index];
//return (int) pageAddress|(pte_index<<2);
...
{
    // TODO
char* pageDirectoryEntry = PDirPool[proc_index][pde_index];

int*  pageTable  = (int) pageDirectoryEntry &  0xfffff000;

pageTable[pte_index] = (page_index<<12)| perm;
//pageTable|(pte_index<<2)= (page_index<<12)| perm;
